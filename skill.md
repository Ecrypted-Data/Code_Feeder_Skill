---
name: code-feeder
description: 当用户需将代码提供给外部 AI（重构/分析/修复）时激活。自动收集代码、记录意图、生成上下文文档。支持增量合并。支持代码清洗（去注释/骨架模式）和垃圾文件过滤。
---
...
4.  **按需清洗**：根据 AI 分析需求选择代码清洗级别，减少无关信息干扰。
...
| 模式 | 标志 | 效果 | 适用场景 |
| :--- | :--- | :--- | :--- |
| **None** | `--clean none` | 不做任何处理（默认） | 需要完整代码 |
| **Comments** | `--clean comments` | 去除注释，保留实现 | 分析代码结构 |
| **Skeleton** | `--clean skeleton` | 只保留函数/类声明 | 快速了解 API |

### 清洗示例
```bash
# 批量导入并去注释
python code_collector.py "project/" --mode batch --files "*.py" --clean comments

# 骨架模式 - 只提取函数声明
python code_collector.py "project/" --mode snippets --target "main.js" \
  --ranges '[{"type":"function","name":"handleClick"}]' --clean skeleton
```

### 垃圾文件过滤
默认自动过滤 STM32/Unity 等自动生成的垃圾文件。可用 `--no-junk-filter` 禁用：
```bash
python code_collector.py "project/" --mode batch --files "*.c" --no-junk-filter
```
...
- **Rust** (`.rs`)
- **C#** (`.cs`)
- **其他**：Ruby, PHP, Swift, SQL, HTML, CSS, JSON, YAML 等